...
def _repair_orphan_tool_calls(messages: list[dict]) -> list[dict]:
    """Give every tool_call in an assistant turn a matching tool result.

    A tool can end without recording a result — it errored, timed out, or the
    model moved on before it returned. Lenient backends ignore the dangling
    tool_call, but Anthropic on Bedrock Converse validates the pairing and
    rejects the whole request with "Expected toolResult blocks at
    messages.N.content for the following Ids: ...", which strands the
    conversation: every later turn replays the same broken history.

    So synthesise a result for each id that has none, keeping it inside the
    contiguous run of tool messages where the backend expects to find it.
    """
    out: list[dict] = []
    i, n = 0, len(messages)
                ...
                out.append(
                    {
                        ...
                        "content": (
                            "Tool call did not complete and returned no result."
                        ),
                    }
                )
...
def _translate_user_blocks(blocks: list[dict], supports_vision: bool = True, vision_format: str = "openai") -> list[dict]:
    ...
    # Images returned BY a tool travel in their own role:user message, but it
    # must not land between two tool results. Strict backends (Anthropic on
    # Bedrock Converse) require every toolResult for one assistant turn to sit
    # in a single contiguous message, and reject the batch with "Expected
    # toolResult blocks at messages.N.content" when something splits it. So
    # hold these aside and emit them once the run of tool results has ended.
    pending_image_msgs: list[dict] = []

    def _flush_pending_images() -> None:
        if pending_image_msgs:
            result.extend(pending_image_msgs)
            pending_image_msgs.clear()
    ...
    for block in blocks:
        ...
        if block.get("type") == "tool_result":
            ...
            if extra_images:
                ...
                pending_image_msgs.append({"role": "user", "content": img_parts})
