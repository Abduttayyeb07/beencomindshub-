...
class TestDataVaultSaveLoad:
    ...
    def test_json_contains_metadata(self, vault):
        # Records are encrypted on disk, so metadata is checked through the
        # decrypting read path rather than by parsing the file directly.
        vault.save("postgresql", "prod_db", {"host": "x"})
        raw = vault.read_record("postgresql", "prod_db")
...
class TestDataVaultEncryption:
    @pytest.fixture(autouse=True)
    def _no_env_key(self, monkeypatch):
        # A key exported in the developer's shell would otherwise leak in.
        monkeypatch.delenv("ANTON_VAULT_KEY", raising=False)

    def test_record_is_not_plaintext_on_disk(self, vault, vault_dir):
        vault.save("postgresql", "prod_db", {"password": "hunter2-secret"})
        raw = (vault_dir / "postgresql-prod_db").read_bytes()
        assert b"hunter2-secret" not in raw
        assert b"postgresql" not in raw  # metadata is encrypted too
        assert vault.load("postgresql", "prod_db") == {"password": "hunter2-secret"}

    def test_key_file_lives_beside_the_vault_dir(self, vault, vault_dir):
        vault.save("postgresql", "prod_db", {"host": "x"})
        assert (vault_dir.parent / ".vault.key").is_file()
        assert not (vault_dir / ".vault.key").exists()

    def test_legacy_plaintext_record_is_read_then_reencrypted(self, vault, vault_dir):
        vault_dir.mkdir(parents=True)
        legacy = vault_dir / "postgresql-old_db"
        legacy.write_text(json.dumps({
            "engine": "postgresql", "name": "old_db", "created_at": "2025-01-01",
            "fields": {"password": "legacy-secret"},
        }))
        assert vault.load("postgresql", "old_db") == {"password": "legacy-secret"}
        assert b"legacy-secret" not in legacy.read_bytes()
        # Still readable after the rewrite.
        assert vault.load("postgresql", "old_db") == {"password": "legacy-secret"}

    def test_wrong_key_reads_as_missing_instead_of_crashing(self, vault, vault_dir, tmp_path):
        from cryptography.fernet import Fernet

        vault.save("postgresql", "prod_db", {"host": "x"})
        stranger = LocalDataVault(vault_dir=vault_dir, key=Fernet.generate_key())
        assert stranger.load("postgresql", "prod_db") is None
        assert stranger.list_connections() == []

    def test_env_key_is_used_and_no_key_file_is_written(self, vault_dir, monkeypatch):
        from cryptography.fernet import Fernet

        monkeypatch.setenv("ANTON_VAULT_KEY", Fernet.generate_key().decode())
        writer = LocalDataVault(vault_dir=vault_dir)
        writer.save("postgresql", "prod_db", {"host": "x"})
        assert not (vault_dir.parent / ".vault.key").exists()
        assert LocalDataVault(vault_dir=vault_dir).load("postgresql", "prod_db") == {"host": "x"}

    def test_sibling_shares_the_key_without_minting_a_new_one(self, vault, tmp_path):
        vault.save("postgresql", "prod_db", {"host": "x"})
        copy_dir = tmp_path / "scratch" / "copy"
        copy = vault.sibling(copy_dir)
        copy.save("postgresql", "prod_db", vault.load("postgresql", "prod_db"))
        assert copy.load("postgresql", "prod_db") == {"host": "x"}
        assert not (copy_dir.parent / ".vault.key").exists()

    def test_resave_overwrites_existing_record(self, vault):
        # Regression: the atomic write used rename(), which on Windows refuses
        # to replace an existing file, so every update of a connection failed.
        vault.save("postgresql", "prod_db", {"host": "a"})
        vault.save("postgresql", "prod_db", {"host": "b"})
        assert vault.load("postgresql", "prod_db") == {"host": "b"}

    def test_list_connections_skips_the_key_and_temp_files(self, vault, vault_dir):
        vault.save("postgresql", "prod_db", {"host": "x"})
        (vault_dir / ".hidden").write_text("{}")
        (vault_dir / "postgresql-half.tmp").write_bytes(b"partial")
        assert [c["name"] for c in vault.list_connections()] == ["prod_db"]
