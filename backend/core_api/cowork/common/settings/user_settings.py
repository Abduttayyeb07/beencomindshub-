...
class UserSettings(Settings):
    ...
    # Model fields that were empty until apply_model_defaults filled them.
    _defaulted_models: frozenset[str] = PrivateAttr(default_factory=frozenset)
    ...
    def apply_model_defaults(self) -> 'UserSettings':
        # Remember which model fields had no value of their own before the
        # provider defaults are filled in below. Afterwards the two cases look
        # identical — model_fields_set can't tell them apart either, since this
        # assignment marks the field as set — and managed mode needs to know
        # whether the user actually picked a model (see chosen_model).
        self._defaulted_models = frozenset(
            name for name in ("planning_model", "coding_model")
    ...
    def chosen_model(self, name: str) -> str | None:
        """The model the user explicitly picked for `name`, or None.

        None when the field only holds the provider default filled in by
        apply_model_defaults, or was saved blank. Under managed mode that
        default (e.g. an Anthropic model id) isn't what should be sent — the
        gateway's role alias is, so the routing policy gets to choose.
        """
        if name in self._defaulted_models:
            return None
    ...
    def config_status(self) -> dict[str, Any]:
        """Whether the active planning provider can be constructed."""
        p = self.planning_provider
        key_field = p.api_key_field
        label = p.label
        if p == Provider.OPENAI_COMPATIBLE:
            ready = bool(self.openai_base_url.strip())
            error = None if ready else "Configure an OpenAI-compatible base URL."
        else:
            ready = getattr(self, key_field, None) is not None
            error = None if ready else f"Configure {key_field} for {label}."
        return {
            "config_ready": ready,
            "config_error": error,
            "provider": p.value,
