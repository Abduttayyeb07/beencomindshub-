...
class ConnectorSettings(Settings):
    ...
    enabled: str = Field(
        default="",
        validation_alias=AliasChoices("COWORK_ENABLED_CONNECTORS"),
        description=(
            "Comma-separated connector ids to offer in the picker. The spec "
            "directory ships the full upstream catalogue; this narrows it to "
            "what this deployment actually uses. Empty means offer everything."
        ),
    )  # COWORK_ENABLED_CONNECTORS

    def allowed_ids(self) -> set[str] | None:
        """The allowlist as a set, or None when no filtering is configured."""
        ids = {p.strip().lower() for p in self.enabled.split(",") if p.strip()}
        return ids or None
