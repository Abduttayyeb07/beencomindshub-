...
def validate_settings(session: SessionDep):
    ...
    if managed.enabled:
        return {
            ...
            "managedModels": True,
            "planningModel": managed.planning_model,
            "codingModel": managed.coding_model,
        }
...
def check_configured(session: SessionDep):
    managed = get_managed_models()
    if managed.enabled:
        return {"configured": True, "provider": "managed-bedrock", "managedModels": True}
    s = SettingService(session).load()
...
async def recommended_models(session: SessionDep):
    ...
    # Under managed mode the gateway's aliases are the only usable models,
    # so they replace the static list for the provider Cowork actually
    # routes through. Both roles draw from the same set.
    if get_managed_models().enabled:
        gateway_models = await fetch_gateway_models()
        if gateway_models:
            recommended["openai-compatible"] = gateway_models
            pair["openai-compatible"] = gateway_models
