...
async def _scheduler_loop() -> None:
    ...
    # Reap schedules that came due while the server was DOWN. This belongs at
    # startup only: _handle_missed_runs disables a past-due `once` schedule
    # without running it, which is right for one that was genuinely missed but
    # wrong for one that just came due. Calling it every tick disabled every
    # one-off the moment it fell due — and because the `due` filter below keys
    # off `enabled`, the run was then skipped entirely, so `once` schedules
    # never executed at all.
    session = get_open_session()
    try:
        _handle_missed_runs(session)
    except Exception:
        logger.exception("Scheduler startup missed-run sweep failed")
    finally:
        session.close()
