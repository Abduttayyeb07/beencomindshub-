...
def _enabled_connector_ids() -> set[str] | None:
    """Connector ids this deployment offers, or None for "all of them".

    The spec directory ships every connector upstream supports, which is far
    more than a given deployment wants in its picker. COWORK_ENABLED_CONNECTORS
    is a comma-separated allowlist of ids; leave it unset to keep the full
    catalogue. Filtering here rather than deleting spec files keeps the choice
    reversible and survives upstream updates to the directory.
    """
    from cowork.common.settings.app_settings import ConnectorSettings

    try:
        return ConnectorSettings().allowed_ids()
    except Exception:
        return None
...
class ConnectorSpecRegistry:
    ...
    def _load_all(self) -> dict[str, dict]:
        ...
        if not self._specs_dir.is_dir():
            return out
        allowed = _enabled_connector_ids()
        for path in sorted(self._specs_dir.glob("*.json")):
            ...
            if allowed is not None and str(cid).lower() not in allowed:
                continue
            data["id"] = cid
            out[cid] = data
        return out
