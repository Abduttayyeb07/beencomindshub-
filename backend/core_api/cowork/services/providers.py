...
_gateway_models_cache: dict[str, tuple[float, Optional[list[str]]]] = {}


async def fetch_gateway_models() -> Optional[list[str]]:
    """List the aliases the managed gateway publishes on `/v1/models`.

    Under managed mode these aliases are the only model names Cowork can
    use, so the Settings picker should offer exactly this list rather than
    the static per-provider defaults. Returns None on any failure, leaving
    the caller on its fallback."""
    from cowork.common.settings.managed_models import get_managed_models

    managed = get_managed_models()
    if not managed.enabled or managed.api_key is None:
        return None
    base = managed.base_url.rstrip("/")

    now = time.monotonic()
    cached = _gateway_models_cache.get(base)
    if cached:
        ts, val = cached
        ttl = _MINDS_MODELS_TTL if val else _MINDS_MODELS_FAIL_TTL
        if (now - ts) < ttl:
    ...
    def _remember(val: Optional[list[str]]) -> Optional[list[str]]:
        _gateway_models_cache[base] = (time.monotonic(), val)
        return val
    ...
    try:
        async with httpx.AsyncClient(
            ...
            r = await client.get(
                f"{base}/models",
                headers={
                    "Authorization": f"Bearer {managed.api_key.get_secret_value()}"
                },
            )
        if r.status_code >= 400:
            logger.debug("gateway /models fetch returned HTTP %s", r.status_code)
            return _remember(None)
        data = r.json()
    except Exception as exc:
        logger.debug("gateway /models fetch failed: %s", exc)
        return _remember(None)
    ...
    return _remember([i for i in ids if i] or None)
...
def build_llm_client():
    ...
    if managed.enabled:
        # The gateway is the only component that has AWS credentials. Cowork
        # receives a restricted internal key and stable model aliases, which
        # keeps Bedrock/IAM details out of the database and browser.
        gateway = OpenAIProvider(
            api_key=managed.api_key.get_secret_value(),
            base_url=managed.base_url.rstrip("/"),
        )
        # The user may pick any alias the gateway publishes; otherwise send the
        # operator's role alias so the gateway's routing policy decides. Only
        # an explicit pick counts — a provider default filled in by
        # UserSettings would silently bypass routing. Only the alias travels;
        # credentials and the real Bedrock model id stay in the gateway.
        return LLMClient(
            planning_provider=gateway,
            planning_model=settings.chosen_model("planning_model") or managed.planning_model,
            coding_provider=gateway,
            coding_model=settings.chosen_model("coding_model") or managed.coding_model,
        )
    ...
    def _make_provider(role: Provider):
        ...
        if role == Provider.OPENAI_COMPATIBLE:
            ...
            return OpenAIProvider(
                api_key=key.get_secret_value() if key is not None else "not-needed",
                base_url=base_url,
            )
