...
class HermesHarness:
    ...
    def _run(
        ...
        *,
        ...
        # Hermes' file tools resolve relative paths against the server process's
        # working directory, and AIAgent takes no cwd argument — so a bare
        # "write me a file" landed in the server's source tree instead of the
        # project. Anton avoids this by running inside a Workspace; here the
        # absolute path has to be stated so the model never emits a bare
        # relative path.
        workdir_context = (
            "## Working directory\n"
            f"The current project directory is `{project_path}`.\n"
            "Always use absolute paths under that directory for file reads and "
            "writes. A relative path does NOT resolve to the project and will "
            "write outside it, so never use one.\n"
        )
        ...
        # Honour the user's explicit pick under managed mode, same as the Anton
        # harness; otherwise send the role alias so the routing policy decides.
        # The value is only a gateway alias — credentials stay server-side.
        model = (
            (settings.chosen_model("planning_model") or managed.planning_model)
            if managed.enabled
            else settings.planning_model
        )
        provider_value = (
            "openai" if managed.enabled or settings.planning_provider == Provider.MINDS_CLOUD
            else settings.planning_provider.value
        )
        api_key_value = (
            managed.api_key.get_secret_value()
            if managed.enabled
            else settings.minds_api_key.get_secret_value()
        ...
        system_context = f"{workdir_context}\n\n{datasource_context}\n\n{artifact_context}"
        ...
        elif settings.planning_provider == Provider.MINDS_CLOUD:
