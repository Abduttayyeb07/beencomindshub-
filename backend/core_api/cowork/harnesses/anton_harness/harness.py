...
def _build_filtered_vault(source_vault, disabled_connections: list[dict], temp_dir: Path, LocalDataVault):
    ...
    # Share the source vault's key: a vault resolves its key from its own
    # location, so a fresh LocalDataVault(temp_dir) would mint a new key file
    # for every request's throwaway copy.
    filtered = source_vault.sibling(temp_dir)
...
class AntonHarness:
    ...
    async def _build_chat_session(
        ...
        # In managed deployments every Anton subsystem (including the
        # scratchpad subprocess) must use the private gateway. The LLMClient
        # is also built from these values below, but the AntonSettings copy is
        # used for capability labels and subprocess environment construction.
        ...
        if managed.enabled:
            ...
            # Keep the user's explicit pick — the gateway resolves it — and
            # otherwise send the role alias so the routing policy decides.
            settings.planning_model = (
                user.chosen_model("planning_model") or managed.planning_model
            )
            settings.coding_model = (
                user.chosen_model("coding_model") or managed.coding_model
            )
            settings.openai_api_key = managed.api_key.get_secret_value()
            settings.openai_base_url = managed.base_url.rstrip("/")
