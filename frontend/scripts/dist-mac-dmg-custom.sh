...
VOL_NAME="${PRODUCT_NAME} Installer ${VERSION}"
OUT_DMG="release/${ARTIFACT_NAME}-${VERSION}-universal-custom.dmg"
...
# Reuse electron-builder's configured DMG background/layout and the signed app.
# Avoid appdmg's obsolete image parser dependency.
npx electron-builder --mac dmg --universal --prepackaged "$APP_DIR" \
  -c.dmg.title="$VOL_NAME" \
  -c.dmg.artifactName="$(basename "$OUT_DMG")"
