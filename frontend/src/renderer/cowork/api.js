  ...
  const op = _settingsLock.then(async () => {
    try {
      ...
      try {
        ...
        result.managedModels = Boolean(v.managedModels);
        if (v.managedModels) {
          result.planningModel = v.planningModel || result.planningModel;
          result.codingModel = v.codingModel || result.codingModel;
        }
